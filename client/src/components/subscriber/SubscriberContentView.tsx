...
const SubscriberContentView: React.FC<SubscriberContentViewProps> = ({ content, onBack }) => {
  ...
  const mockThreads: Thread[] = [
    {
      ...
      messages: [
        ...
        { role: 'assistant', content: 'Welcome! This content covers the basics of machine learning. You can start by exploring the first module on data preprocessing.' }
      ],
      metaInfo: {},
    ...
    },
    {
      ...
      messages: [
        ...
        { role: 'assistant', content: 'Neural networks are computational models inspired by the human brain. They consist of layers of interconnected nodes or "neurons" that process information.' }
      ],
      metaInfo: {},
    ...
    const fetchThreads = async () => {
      ...
      try {
        ...
        if (mockThreads.length > 0 && !activeThreadId) {
          ...
          setMessages(mockThreads[0].messages);
        }

        // Actual API implementation (commented out)
        // const fetchedThreads = await ThreadAPI.listByContent(content.id);
  ...
  const handleUserMessage = (message: string) => {
    ...
    //     
    //     const response = await ThreadAPI.update(activeThreadId, {
    //       messages: [newMessage],
    //       append: true,
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-200">
      ...
      <main className="container mx-auto px-4 py-4">
        <div className="flex h-[calc(100vh-140px)]">
          ...
          {/* Chat area */}
          <div className="flex-grow bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 transition-colors duration-200">
            {activeThread ? (
              ...
              <div className="h-full flex flex-col items-center justify-center text-center p-6">
                <MessageSquare size={48} className="text-gray-300 dark:text-gray-600 transition-colors duration-200 mb-3" />
                <h3 className="text-xl font-medium text-gray-700 dark:text-gray-300 transition-colors duration-200 mb-2">
                  No Active Thread
                </h3>
